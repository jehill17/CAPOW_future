...
def PNW_demand(BPA_demand):
    ...
    df_area = pd.read_excel('Synthetic_demand_pathflows/AreaLoadShapes.xlsx', usecols=[1,5,7,12,14,22,26,29,30,35])
    ...
    years = int(len(BPA_demand)/8760)
    
    for y in range(0,years):
        
        load = np.squeeze(BPA_demand[y*8760:y*8760+8760])
        
        BPA_std_change = np.std(load)/np.std(BPA2024_demand)
        BPA_mean_change = np.mean(load)/np.mean(BPA2024_demand)
    ...
    #    CHPD_hist_peak = 500
        ...
        Total_PNW_load = np.sum((AVA_demand,load,CHPD_demand,DOPD_demand,GCPD_demand,PACW_demand,PGE_demand,PSEI_demand,SCL_demand,TPWR_demand),axis=0)
    
        if y < 1:
            T = Total_PNW_load
        else:
            T = np.append(T,Total_PNW_load)
